...
void draw_glyph(enum displayGlyph c, SDL_Rect rect, uint8_t r, uint8_t g, uint8_t b) {
    ...
    if (c < MIN_TILE) {
        ...
        switch(c){
            ...
            convert_glyph(G_TURRET,128+9,350)
            ...
            convert_glyph(G_ELECTRIC_CRYSTAL,164,361)
            ...
            convert_glyph(G_GOOD_ITEM,128 + 13,382)
            convert_glyph(G_BAD_ITEM,128 + 12,383)
            convert_glyph(G_AMULET_ITEM,128+5,384)
            convert_glyph(G_NET, ':', 385)
            convert_glyph(G_FALLEN_TORCH, '#', 386)
            convert_glyph(G_PRESSURE_PLATE_INACTIVE, 128 + 2, 387)
            convert_glyph(G_TRAP_GAS, 128 + 2, 388)
            convert_glyph(G_TRAP_PARALYSIS, 128 + 2, 389)
            convert_glyph(G_TRAP_CONFUSION, 128 + 2, 390)
            convert_glyph(G_TRAP_FIRE, 128 + 2, 391)
            convert_glyph(G_TRAP_FLOOD, 128 + 2, 392)
            convert_glyph(G_TRAP_NET, 128 + 2, 393)
            convert_glyph(G_PRESSURE_PLATE, 128 + 2, 394)
            convert_glyph(G_SACRED_GLYPH,128+1,395)
            convert_glyph(G_GLOWING_GLYPH,128+1,396)
            convert_glyph(G_CRYSTAL_WALL,'#',397)
            default: key = '?';
        }
    }
